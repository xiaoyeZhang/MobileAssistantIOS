//
//  SA_Special_SumbitViewController.m
//  MobileAssistant
//
...
#import "SA_SpecialViewController.h"
#import "M_OrderUserViewController.h"

@interface SA_Special_SumbitViewController ()<UITextFieldDelegate,MBProgressHUDDelegate,M_Product_listViewControllerDelegate,M_OrderUserViewControllerDelegate>
{
...
@implementation SA_Special_SumbitViewController
                    ...
                    if ([self.uploadImagesArr[i] isKindOfClass:[UIImage class]]) {
                        ...
                        if (i == [self.uploadImagesArr count] - 1) {
                            ...
                            [alert showAlertViewWithCompleteBlock:^(NSInteger buttonIndex) {
                                ...
                                SA_SpecialViewController *vc = [[SA_SpecialViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == _listArr.count){
        ...
        if ([entity.type_id isEqualToString:@"1"]) {
            ...
            //            cell.txtField.text = entity.data_info;
...
- (void)getDate{
    ...
    [dic setObject:@"m_get_order_form" forKey:@"method"];
    [dic setObject:userEntity.user_id forKey:@"user_id"];
    [dic setObject:_product_name forKey:@"product_name"];
    [dic setObject:@"3" forKey:@"type"];
    
    if ([_type isEqualToString:@"1"]) {
        [dic setObject:@"新增集团" forKey:@"company_name"];
        [dic setObject:@"000000" forKey:@"company_num"];
    }else{
        [dic setObject:_entity.name forKey:@"company_name"];
        [dic setObject:_entity.num forKey:@"company_num"];
...
- (void)uploadImagesWithIndex:(int)index
{
    ...
    [manager POST:BASEURL parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //        for (int i = 0; i < _uploadImagesArr.count; i++) {
        //
        //            UIImage *image = _uploadImagesArr[i];
        //
        //            NSData *imageData = UIImageJPEGRepresentation(image, 0.5f);
        //
        //            [formData appendPartWithFileData:imageData name:@"file" fileName:_uploadImagesNameArr[i] mimeType:@"image/jpg/png/jpeg"];
        //        }
        //
        //对于图片进行压缩
        ...
        [formData appendPartWithFileData:data name:@"file" fileName:[NSString stringWithFormat:@"%@",imageName] mimeType:@"image/jpg/png/jpeg"];
        ...
        if ([[obj objectForKey:@"state"] intValue]== 1) {
            ...
            UIAlertView* alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"图片上传未成功!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert showAlertViewWithCompleteBlock:^(NSInteger buttonIndex) {
