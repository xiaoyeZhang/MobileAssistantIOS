//
//  News_ProviceVIP_SubmitViewController.m
//  MobileAssistant
//
...
@interface News_ProviceVIP_SubmitViewController ()<UITextFieldDelegate,News_CustomerViewControllerDelegate,MBProgressHUDDelegate,CheckBoxTableViewCellDelegate,ThreeCheckBoxTableViewCellDelegate>{
...
@implementation News_ProviceVIP_SubmitViewController
...
- (void)submitBtnClicked:(id)sender
{
    ...
    for ( News_ProvinceVIP_SubmitEntity *entity in self.arrayCutomer) {
        ...
        if (![entity.hidden_info isEqualToString:@"-1"]) {
            ...
            if ([[submitDic objectForKey:entity.submit_num] isEqualToString:arr[0]]) {
                ...
                [submitDic removeObjectForKey:arr[1]];
                continue;
            }
        }
    ...
    for (NSString *key in submitDic) {
        ...
        if (str.length == 0) {
            ...
            for ( News_ProvinceVIP_SubmitEntity *entity in self.arrayCutomer) {
                if ([entity.submit_num isEqualToString:key] && ![entity.form_type isEqualToString:@"5"] && ![entity.form_type isEqualToString:@"6"] && [entity.constraint isEqualToString:@"-1"]) {
    ...
    [submitDic setObject:@"m_submit_business" forKey:@"method"];
    [submitDic setObject:self.module_id forKey:@"module_id"];
    ...
    [service getNetWorkData:submitDic Successed:^(id entity) {
        ...
        if (state > 0) {
            ...
            if ([type isEqualToString:@"5"]) {
                ...
                [self uploadImagesWithIndex:0 withBusinessId:entity[@"state"] andwithSubmit_num:submit_num];
    ...
    [manager POST:BASEURL parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([[obj objectForKey:@"state"] intValue]== 1) {
            if ([dict[@"state"] intValue] == 1) {
                ...
                UIAlertView* alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"上传图片失败" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                [alert showAlertViewWithCompleteBlock:^(NSInteger buttonIndex) {
                    [self.navigationController popViewControllerAnimated:YES];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGSize size1 = [cell1.titleLbl sizeThatFits:CGSizeMake(cell1.titleLbl.frame
                                                           .size.width, MAXFLOAT)];
    if (size1.height == 0) {
        return 44;
    }
...
//    }
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            News_ProvinceVIP_SubmitEntity *entity = [self.arrayCutomer objectAtIndex:indexPath.row - 2];
            ...
            for (NSString *key in submitDic) {
                ...
                if ([entity.submit_num isEqualToString:key] && ![entity.form_type isEqualToString:@"6"]) {
                    cell1.txtField.text = [submitDic objectForKey:key];
                }
            }
            
            if ([entity.submit_num isEqualToString:@"gz_jtbh"]) {
                
                cell1.txtField.placeholder = @"选择集团单位后自动生成";
            }
            ...
            if ([entity.form_type isEqualToString:@"1"]) {
                ...
                if (arr.count == 1) {
                    ...
                    [cell4.clickBtn addTarget:self action:@selector(checkBtn:) forControlEvents:UIControlEventTouchUpInside];
                ...
                cell1.txtField.placeholder = @"请选择";
                cell1.downArrowImageView.hidden = NO;
                
            }else if ([entity.form_type isEqualToString:@"3"]) {
                
                cell1.txtField.placeholder = [NSString stringWithFormat:@"请选择%@",entity.name];
                
            }else if ([entity.form_type isEqualToString:@"4"]) {
                
                cell1.txtField.placeholder = @"请选择日期";
                cell1.txtField.text = self.order_time;
                
            }else if ([entity.form_type isEqualToString:@"5"]){
                
                cell1.txtField.placeholder = @"请上传合同相关资料";
                cell1.txtField.text = self.uploadImagesArr.count>0?@"查看上传资料":nil;
                
            }else if ([entity.form_type isEqualToString:@"6"]){
                
                cell1.txtField.placeholder = @"添加机型";
    ...
    [service getNetWorkData:dic Successed:^(id entity) {
        ...
        if ([strState isEqualToString:@"0"]) {
            ...
            for (NSDictionary *attributes in arr) {
                ...
                if ([entity.data_source_type isEqualToString:@"0"] && ![entity.data_source isEqualToString:@"-1"] && [entity.form_type isEqualToString:@"0"]) {
                    
                    [submitDic setObject:entity.data_source forKey:entity.submit_num];
...
//                    NSArray *arr = [entity.data_source componentsSeparatedByString:@";"];
//                    
//                    [submitDic setObject:arr[0] forKey:entity.submit_num];
//                }
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    News_ProvinceVIP_SubmitEntity *entity = [self.arrayCutomer objectAtIndex:textField.tag - 2];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    News_ProvinceVIP_SubmitEntity *entity;
    ...
    if ([entity.form_type intValue] == FORM_TYPE_ZERO) {
        ...
        vc.delegate = self;
//        date_table_index = textField.tag;
        [self.navigationController pushViewController:vc animated:YES];
...
{
    News_ProvinceVIP_SubmitEntity *entity = [self.arrayCutomer objectAtIndex:cell.titleLbl.tag - 2];
    NSArray *arr = [entity.data_source componentsSeparatedByString:@";"];
    if (selectedIndex == 1) {
        [submitDic setValue:arr[0] forKey:entity.submit_num];
        self.isCheckBoxUnPass = NO;
...
- (void)checkBtn:(UIButton *)sender{
    ...
    News_ProvinceVIP_SubmitEntity *entity = [self.arrayCutomer objectAtIndex:sender.tag - 2];
    NSArray *arr = [entity.data_source componentsSeparatedByString:@";"];
    
    if (sender.selected) {
        
//        [cell4.clickBtn setImage:[UIImage imageNamed:@"check_normal"] forState:UIControlStateNormal];
//        sender.selected = NO;
        NSLog(@"未选中状态");
//
