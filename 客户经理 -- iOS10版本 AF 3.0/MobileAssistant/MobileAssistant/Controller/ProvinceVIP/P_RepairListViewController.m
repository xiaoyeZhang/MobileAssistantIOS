...
@interface P_RepairListViewController ()

@end

@implementation P_RepairListViewController
...
- (BOOL)checkIsShowNewTagWithIndex:(int)index
{
    ...
    if ([model.type_id isEqualToString:@"6"]) {  //大于1000
        if (state == PROCESS_STATE_manager_submit &&
            ...
            return YES;
                  ...
                  (userType == ROLE_REPAIR || userType == ROLE_COMMON)){ //二级经理审批 -> 综合处理
            return YES;
        }
        ...
        if (state == PROCESS_STATE_manager_submit &&
            ...
            return YES;
                  ...
                  (userType == ROLE_REPAIR || userType == ROLE_COMMON)){ //三级经理审批 -> 综合处理
            return YES;
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    P_RepairDetailViewController *vc = [[P_RepairDetailViewController alloc] initWithNibName:@"P_BusinessDetailBaseViewController" bundle:nil];
    vc.bListModel = model;
    [self.navigationController pushViewController:vc animated:YES];
}
