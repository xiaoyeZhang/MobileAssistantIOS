...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            AdEntity *ad = [imageArray objectAtIndex:i];
            //NSString *imageUrl = [NSString stringWithFormat:@"%@%@", IMAGE_URL, ad.filename];
            if ([ad.filename hasPrefix:@"http://"]) {
                //网络图片 请使用ego异步图片库
                [imgView setImageWithURL:[NSURL URLWithString:ad.filename]];
            }
            else {
                UIImage *img=[UIImage imageNamed:ad.filename];
...
- (void)scrollViewDidScroll:(UIScrollView *)sender
{
    ...
    int page = pageControl.currentPage;//floor((scrollView.contentOffset.x - pageWidth / 2) / pageWidth) + 1;
