...
-(void)selectIndex:(int)index{
    [self selectIndex:index annimation:YES];
}
-(void)selectIndex:(int)index annimation:(BOOL)animation{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (finished) {
            //设置新页出现
            if (!_isRootScroll) {
                [_rootScrollView setContentOffset:CGPointMake(index*self.bounds.size.width, 0) animated:animation];
            }
            _isRootScroll = NO;
